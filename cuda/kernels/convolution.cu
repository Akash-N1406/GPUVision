...
DetailedTimingSamples convolution_cuda_naive_detailed(const Image& img,
    ...
    if (kernel_size % 2 == 0) {
        throw std::runtime_error("convolution_cuda_naive_detailed: kernel_size must be odd");
    }
    if (static_cast<int>(kernel.size()) != kernel_size * kernel_size) {
        throw std::runtime_error("convolution_cuda_naive_detailed: kernel size mismatch");
    }

    const size_t img_bytes = img.size_bytes();
    const size_t kernel_bytes = kernel.size() * sizeof(float);

    // Allocated ONCE, reused across every warmup and measured iteration —
    // this is the key difference from convolution_cuda_naive, which
    // allocates fresh buffers on every single call.
    uint8_t* d_in = nullptr;
    uint8_t* d_out = nullptr;
    float* d_kernel = nullptr;
    CUDA_CHECK(cudaMalloc(&d_in, img_bytes));
    CUDA_CHECK(cudaMalloc(&d_out, img_bytes));
    CUDA_CHECK(cudaMalloc(&d_kernel, kernel_bytes));
    // Kernel weights don't change between iterations, so upload once,
    // outside the timed loop entirely.
    CUDA_CHECK(cudaMemcpy(d_kernel, kernel.data(), kernel_bytes, cudaMemcpyHostToDevice));
    ...
    // All four events sit on the default stream, same as the memcpy/kernel
    // calls, so they execute in strict issue order — a single
    // cudaEventSynchronize on the last event is enough to know the whole
    // chain has completed, and cudaEventElapsedTime between consecutive
    // pairs gives an accurate GPU-clock-timed breakdown of each phase.
    auto run_once = [&]() {
